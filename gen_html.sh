...
do
 ...
 if [ $i = ${ALL_PLAT[${pj}]} ];then
  ...
  mutt -s "mx$i ${OUTPUT_FILE} board test result" -a ${OUT_BASE}/LTP_RUN_ON-${OUTPUT_FILE}.failed lbgtest@lists.shlx12.ap.freescale.net BSPTEST@freescale.com < "please see http://shlx12.ap.freescale.net/test_reports/" 
	fi
 fi	
 pj=$(expr $pj + 1)
done
